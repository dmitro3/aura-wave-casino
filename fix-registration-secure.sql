-- SECURE REGISTRATION FIX - RUN THIS DIRECTLY ON PRODUCTION DATABASE
-- This script fixes registration while maintaining proper security
-- Re-enables RLS with proper policies that allow registration to work
...
-- User level stats table policies
...
CREATE POLICY "user_level_stats_insert_own" ON public.user_level_stats
  FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Service role policies for user_level_stats (needed for trigger function)
CREATE POLICY "user_level_stats_service_role_select" ON public.user_level_stats
  FOR SELECT USING (auth.role() = 'service_role');

CREATE POLICY "user_level_stats_service_role_insert" ON public.user_level_stats
  FOR INSERT WITH CHECK (auth.role() = 'service_role');

CREATE POLICY "user_level_stats_service_role_update" ON public.user_level_stats
  FOR UPDATE USING (auth.role() = 'service_role');
...
CREATE POLICY "admin_users_service_role_select" ON public.admin_users
  FOR SELECT USING (auth.role() = 'service_role');

CREATE POLICY "admin_users_service_role_insert" ON public.admin_users
  FOR INSERT WITH CHECK (auth.role() = 'service_role');

CREATE POLICY "admin_users_service_role_update" ON public.admin_users
  FOR UPDATE USING (auth.role() = 'service_role');
...
CREATE POLICY "notifications_service_role_insert" ON public.notifications
  FOR INSERT WITH CHECK (auth.role() = 'service_role');

-- Case rewards table policies
CREATE POLICY "case_rewards_select_own" ON public.case_rewards
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "case_rewards_service_role_insert" ON public.case_rewards
  FOR INSERT WITH CHECK (auth.role() = 'service_role');

-- 4. CREATE SECURE TRIGGER FUNCTION
...
BEGIN
  ...
  -- Insert into user_level_stats (with proper error handling)
  BEGIN
...
-- 8. TEST THE SECURE FIX
DO $$
DECLARE
  test_user_id UUID := gen_random_uuid();
  test_created_at TIMESTAMP WITH TIME ZONE := now();
BEGIN
  RAISE NOTICE 'Testing secure registration fix...';
