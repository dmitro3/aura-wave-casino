...
-- Run this script in your Supabase SQL Editor to implement the full system

-- ============================================
-- STEP 1: Enhance achievements table structure
-- ============================================

-- Add new fields to achievements table
DO $$ 
BEGIN
  -- Add rarity column
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'achievements' AND column_name = 'difficulty') THEN
    ALTER TABLE public.achievements 
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'achievements' AND column_name = 'reward_amount') THEN
    ALTER TABLE public.achievements 
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'achievements' AND column_name = 'reward_type') THEN
    ALTER TABLE public.achievements 
    ADD COLUMN reward_type TEXT NOT NULL DEFAULT 'money' 
    CHECK (reward_type IN ('money', 'xp', 'cases'));
  END IF;

  RAISE NOTICE '✅ Enhanced achievements table structure';
END $$;

-- ============================================
-- STEP 2: Add enhanced Tower game statistics
-- ============================================
...
BEGIN
  -- Tower highest level
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'user_level_stats' AND column_name = 'tower_highest_level') THEN
    ALTER TABLE public.user_level_stats
    ADD COLUMN tower_highest_level INTEGER NOT NULL DEFAULT 0;
  END IF;

  -- Tower biggest win
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'user_level_stats' AND column_name = 'tower_biggest_win') THEN
    ALTER TABLE public.user_level_stats
    ADD COLUMN tower_biggest_win NUMERIC NOT NULL DEFAULT 0;
  END IF;

  -- Tower biggest loss
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'user_level_stats' AND column_name = 'tower_biggest_loss') THEN
    ALTER TABLE public.user_level_stats
    ADD COLUMN tower_biggest_loss NUMERIC NOT NULL DEFAULT 0;
  END IF;

  -- Tower best streak
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'user_level_stats' AND column_name = 'tower_best_streak') THEN
    ALTER TABLE public.user_level_stats
    ADD COLUMN tower_best_streak INTEGER NOT NULL DEFAULT 0;
  END IF;

  -- Tower current streak
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'user_level_stats' AND column_name = 'tower_current_streak') THEN
    ALTER TABLE public.user_level_stats
    ADD COLUMN tower_current_streak INTEGER NOT NULL DEFAULT 0;
  END IF;

  -- Tower perfect games
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'user_level_stats' AND column_name = 'tower_perfect_games') THEN
    ALTER TABLE public.user_level_stats
    ADD COLUMN tower_perfect_games INTEGER NOT NULL DEFAULT 0;
  END IF;
...
-- ============================================
-- STEP 3: Create achievement reward function
-- ============================================
...
BEGIN
  ...
  IF p_reward_type = 'money' THEN
    ...
    RAISE NOTICE 'Awarded $% to user %', p_reward_amount, p_user_id;
  ...
  ELSIF p_reward_type = 'xp' THEN
    ...
    RAISE NOTICE 'Awarded % XP to user %', p_reward_amount, p_user_id;
  ...
  ELSIF p_reward_type = 'cases' THEN
    ...
    RAISE NOTICE 'Awarded % cases to user %', p_reward_amount, p_user_id;
  END IF;
...
RAISE NOTICE '✅ Achievement reward function created';

-- ============================================
-- STEP 4: Clear existing achievements and insert 30 new ones
-- ============================================

-- Clear existing achievements (careful!)
TRUNCATE public.achievements CASCADE;
TRUNCATE public.user_achievements CASCADE;

-- Insert 30 comprehensive achievements
...
RAISE NOTICE '✅ Inserted 30 comprehensive achievements with different rarities and difficulties';

-- ============================================
-- STEP 5: Create achievement checking function
-- ============================================
...
BEGIN
  ...
  LOOP
    ...
    THEN
      ...
      RAISE NOTICE 'Unlocked achievement: % for user %', achievement.name, p_user_id;
    END IF;
  END LOOP;
...
RAISE NOTICE '✅ Achievement checking function created';

-- ============================================
-- STEP 6: Create automatic achievement trigger
-- ============================================
...
RAISE NOTICE '✅ Automatic achievement checking trigger enabled';
...
BEGIN
  ...
  SELECT COUNT(*) INTO common_count FROM public.achievements WHERE rarity = 'common';
  SELECT COUNT(*) INTO rare_count FROM public.achievements WHERE rarity = 'rare';
  SELECT COUNT(*) INTO epic_count FROM public.achievements WHERE rarity = 'epic';
  SELECT COUNT(*) INTO legendary_count FROM public.achievements WHERE rarity = 'legendary';
  SELECT SUM(reward_amount) INTO total_rewards FROM public.achievements WHERE reward_type = 'money';

  RAISE NOTICE '';
  RAISE NOTICE '🎉 COMPREHENSIVE ACHIEVEMENTS SYSTEM INSTALLED! 🎉';
  RAISE NOTICE '';
  RAISE NOTICE '📊 SYSTEM OVERVIEW:';
  RAISE NOTICE '   💎 Total Achievements: %', achievement_count;
  RAISE NOTICE '   ⚪ Common: % achievements', common_count;
  RAISE NOTICE '   🔵 Rare: % achievements', rare_count;
  RAISE NOTICE '   🟣 Epic: % achievements', epic_count;
  RAISE NOTICE '   🟠 Legendary: % achievements', legendary_count;
  RAISE NOTICE '   💰 Total Money Rewards: $%', total_rewards;
  RAISE NOTICE '';
  RAISE NOTICE '🚀 NEW FEATURES:';
  RAISE NOTICE '   ✅ Enhanced Tower game statistics tracking';
  RAISE NOTICE '   ✅ 30 diverse achievements with 4 rarity levels';
  RAISE NOTICE '   ✅ 4 difficulty tiers (easy, medium, hard, extreme)';
  RAISE NOTICE '   ✅ Automatic achievement detection and rewards';
  RAISE NOTICE '   ✅ Progress tracking for locked achievements';
  RAISE NOTICE '   ✅ Privacy controls (own vs others profiles)';
  RAISE NOTICE '';
  RAISE NOTICE '🎯 WHAT''S NEW FOR USERS:';
  RAISE NOTICE '   📈 Tower stats: highest level, perfect games, streaks';
  RAISE NOTICE '   🏆 Achievement progress bars on own profile';
  RAISE NOTICE '   📅 Unlock dates visible on achievements';
  RAISE NOTICE '   💎 Rarity-based visual design';
  RAISE NOTICE '   🎁 Instant reward payouts';
  RAISE NOTICE '';
  RAISE NOTICE '⚡ The system is now LIVE and ready to use!';
END $$;