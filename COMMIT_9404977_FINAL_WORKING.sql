-- =====================================================================
-- COMMIT 9404977 FINAL WORKING RESTORATION
-- =====================================================================
-- This script restores the database to the EXACT state of commit 9404977
-- FINAL VERSION: Drops all existing functions first to avoid conflicts
-- =====================================================================

BEGIN;

-- =====================================================================
-- STEP 0: DROP ALL EXISTING FUNCTIONS TO AVOID CONFLICTS
-- =====================================================================

-- Drop all functions that might conflict
DROP FUNCTION IF EXISTS public.reset_user_stats_comprehensive(UUID) CASCADE;
...
RAISE NOTICE '🗑️ Dropped all existing functions to avoid conflicts';
...
CREATE TABLE IF NOT EXISTS public.admin_users (
  ...
  permissions text[] NOT NULL DEFAULT ARRAY['crash_control'::text],
...
-- Create crash_rounds table first (referenced by crash_bets)
...
-- Create profiles table first (referenced by notifications)
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  badges text[] NOT NULL DEFAULT ARRAY['welcome'::text],
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  avatar_url text,
...
BEGIN 
  ...
  -- Fix badges column type if it exists as wrong type
  BEGIN
    ALTER TABLE public.profiles ALTER COLUMN badges TYPE text[] USING badges::text[];
  EXCEPTION
    WHEN OTHERS THEN
...
-- Create roulette_rounds table first (referenced by other roulette tables)
...
-- Create the exact function from commit 9404977 (no conflicts now)
...
-- =====================================================================
-- STEP 3: CREATE ESSENTIAL ROULETTE SYSTEM FUNCTIONS (FRESH VERSIONS)
-- =====================================================================

-- Create atomic bet balance check function (fresh version)
...
-- Create rollback function (fresh version)
CREATE FUNCTION public.rollback_bet_balance(
  p_user_id UUID,
  p_bet_amount NUMERIC
)
RETURNS BOOLEAN
...
-- =====================================================================
-- COMMIT 9404977 FINAL WORKING RESTORATION COMPLETE!
-- =====================================================================
-- This script has restored:
-- 
-- ✅ DROPPED ALL EXISTING FUNCTIONS FIRST (no conflicts!)
