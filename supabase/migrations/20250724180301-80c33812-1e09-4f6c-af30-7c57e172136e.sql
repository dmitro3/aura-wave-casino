-- Update the user stats function to give cases every level up with proper rarity progression
...
BEGIN
  ...
  IF new_level_val > old_level_val AND new_level_val <= 1000 THEN
    ...
    FOR i IN (old_level_val + 1)..new_level_val LOOP
      ...
      -- Determine case rarity based on level
      -- Every 25 levels: Legendary
      -- Every 10 levels (but not 25): Epic  
      -- All other levels: Common (rare cases removed for now to focus on progression)
      IF i % 25 = 0 THEN
      ...
      VALUES (p_user_id, i, case_rarity, 0);
...
-- Add DELETE policy for notifications so users can delete their notifications
CREATE POLICY "Users can delete their own notifications" 
ON public.notifications 
FOR DELETE 
USING (auth.uid() = user_id);

-- Update the level calculation function to support up to 1000 levels with reasonable XP scaling
...
BEGIN
  ...
  FOR level_check IN 2..1000 LOOP  -- Support up to level 1000
