-- Create a function to add to live feed with delay for coinflip games
...
BEGIN
  -- Only delay for coinflip games, others are immediate
  IF NEW.game_type = 'coinflip' THEN
    -- For coinflip, we'll add to live feed from the client side after animation
    -- So we don't add it here automatically
    RETURN NEW;
  END IF;
