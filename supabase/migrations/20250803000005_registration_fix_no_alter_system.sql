-- REGISTRATION FIX WITHOUT ALTER SYSTEM
...
-- Step 3: Create trigger WITHOUT WHEN condition
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW 
  EXECUTE FUNCTION public.handle_new_user();

-- Step 4: Fixed manual function with proper parameter naming
...
BEGIN
  ...
  RAISE NOTICE '🔧 REGISTRATION FIX COMPLETED (without ALTER SYSTEM)';
  ...
  RAISE NOTICE '🚀 Trigger fires on ALL auth.users inserts (no WHEN condition)';
