-- Migration: Fix level system with fixed XP requirements
-- This ensures XP requirements never change and level is calculated from total XP

-- Drop the dynamic XP calculation functions
...
-- Create fixed XP lookup function (replaces dynamic calculation)
...
-- Grant permissions
GRANT SELECT ON public.level_xp_requirements TO anon, authenticated, service_role;
...
BEGIN
  -- Test level 1 (0 XP)
  SELECT * INTO test_result FROM calculate_level_from_xp_new(0);
  RAISE NOTICE 'Level from 0 XP: level=%, current_level_xp=%, xp_to_next=%', test_result.level, test_result.current_level_xp, test_result.xp_to_next;
  
  -- Test level 2 (651 XP)
  SELECT * INTO test_result FROM calculate_level_from_xp_new(651);
  RAISE NOTICE 'Level from 651 XP: level=%, current_level_xp=%, xp_to_next=%', test_result.level, test_result.current_level_xp, test_result.xp_to_next;
