-- 🔧 FIX DOUBLE TRACKING, XP CALCULATION & LEVEL REQUIREMENTS
-- Fix statistics being tracked twice, correct XP to 10% of wager, update level requirements

BEGIN;

DO $$ BEGIN RAISE NOTICE '🔧 Fixing double tracking, XP calculation, and level requirements...'; END $$;

-- 1. First, update level_xp_requirements with EXACT corrected values
...
DO $$ BEGIN RAISE NOTICE '✅ Updated level_xp_requirements with corrected values'; END $$;

-- 2. DISABLE all game-specific triggers to prevent double tracking
-- The game_history trigger will be the ONLY source of XP and stats tracking
...
-- 3. Update XP awarding function to give 10% of wager amount
...
-- 4. Keep ONLY the game_history trigger for consistent tracking
-- This ensures each game is tracked exactly once when it completes
...
-- 6. Recalculate all existing user levels with corrected system
UPDATE public.user_level_stats
SET 
...
-- 7. Test the corrected XP calculation
DO $$
DECLARE
  test_user_id uuid;
  test_result RECORD;
  ...
  new_xp numeric;
BEGIN
  RAISE NOTICE '🧪 Testing corrected XP calculation (10% of wager)...';
  ...
  IF test_user_id IS NOT NULL THEN
    -- Get old XP
    SELECT COALESCE(lifetime_xp, 0) INTO old_xp 
    FROM public.user_level_stats 
    WHERE user_id = test_user_id;
    
    -- Test XP awarding: $1 wager should give 0.1 XP
    PERFORM public.award_xp_for_wager(test_user_id, 1.00);
    ...
    RAISE NOTICE 'XP Test: $1.00 wager | Old XP: % | New XP: % | Awarded: %', 
      old_xp, new_xp, (new_xp - old_xp);
    
    -- Test smaller wager: $0.10 should give 0.01 XP
    SELECT lifetime_xp INTO old_xp 
    FROM public.user_level_stats 
    WHERE user_id = test_user_id;
    
    PERFORM public.award_xp_for_wager(test_user_id, 0.10);
    
    SELECT lifetime_xp INTO new_xp 
    FROM public.user_level_stats 
    WHERE user_id = test_user_id;
    
    RAISE NOTICE 'XP Test: $0.10 wager | Old XP: % | New XP: % | Awarded: %', 
      old_xp, new_xp, (new_xp - old_xp);
    ...
    RAISE NOTICE 'Final Stats: Level %, Lifetime XP %, Current Level XP %, XP to Next %', 
      test_result.current_level, test_result.lifetime_xp, test_result.current_level_xp, test_result.xp_to_next_level;
  ELSE
    RAISE NOTICE 'No test users found - system ready for use';
  END IF;
END $$;

DO $$ BEGIN RAISE NOTICE '✅ Fixed double tracking, corrected XP to 10% of wager, updated level requirements!'; END $$;
