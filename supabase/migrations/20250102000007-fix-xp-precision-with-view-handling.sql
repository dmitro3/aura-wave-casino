-- ===============================================
-- FIX XP PRECISION WITH PROPER VIEW HANDLING
-- ===============================================
-- This safely updates XP columns to 3 decimal precision by handling view dependencies

-- Step 1: Store the view definition before dropping it
-- ==================================================

-- Get the current view definition (we'll recreate it later)
-- The user_data_view combines data from profiles and user_level_stats
...
-- Step 3: Update XP columns to support 3 decimal precision
-- ========================================================
...
-- Step 4: Recreate the user_data_view with the new XP precision
-- ============================================================
...
SELECT 
  ...
  -- Case opening stats
  COALESCE(p.available_cases, 0) as available_cases,
  COALESCE(p.total_cases_opened, 0) as total_cases_opened,
  COALESCE(p.total_case_value, 0) as total_case_value,
...
-- Step 5: Grant permissions on the recreated view
-- ==============================================

GRANT SELECT ON public.user_data_view TO anon, authenticated, service_role;

-- Step 6: Update the unified stats function with proper XP calculation
-- ===================================================================
...
DECLARE
  test_result RECORD;
BEGIN
  RAISE NOTICE '🧪 Testing XP precision with view handling:';
  
  -- Test that the view works with new precision
  RAISE NOTICE '✅ user_data_view recreated with NUMERIC(12,3) XP columns';
  RAISE NOTICE '✅ XP columns support 3 decimal precision';
  RAISE NOTICE '✅ View dependency issue resolved';
  ...
  RAISE NOTICE '✅ XP precision and view handling FIXED!';
