...
-- Make calculate_level_from_xp use the exact level_xp_requirements table values
-- and calculate user levels properly from lifetime_xp

BEGIN;

DO $$ BEGIN RAISE NOTICE '🔧 Fixing level calculation to use exact XP requirements...'; END $$;
...
-- 2. Completely recreate the calculate_level_from_xp function to use the exact table values
...
BEGIN
  ...
  LOOP
    ...
    IF remaining_xp >= level_row.xp_required THEN
      remaining_xp := remaining_xp - level_row.xp_required;
      user_level := level_row.level;
      current_xp := 0;  -- We completed this level
    ELSE
      -- We're currently in this level
      current_xp := remaining_xp;
      next_level_xp := level_row.xp_required - remaining_xp;
...
DO $$ BEGIN RAISE NOTICE '✅ Level calculation fixed to use exact XP requirements from table'; END $$;
