-- Add user level achievement and support for user level tracking
-- This adds a user level achievement and updates the checking function

-- Add a user level achievement
...
('level_master', 'Level Master', 'Reach level 50', 'general', 'Star', 'epic', 'hard', 1000, 'money', '{"type": "user_level", "value": 50}'),
('level_legend', 'Level Legend', 'Reach level 100', 'general', 'Crown', 'legendary', 'extreme', 5000, 'money', '{"type": "user_level", "value": 100}');

-- Update the achievement checking function to include user level
...
BEGIN
  ...
  LOOP
    ...
    IF (criteria->>'type' = 'total_games' AND user_stats.total_games >= (criteria->>'value')::INTEGER) OR
       ...
       -- NEW: Add user level achievement support
...
RAISE NOTICE '✅ Added user level achievements and support!';
RAISE NOTICE '🎯 New achievements: Level Master (level 50), Level Legend (level 100)';
RAISE NOTICE '📊 Now supports user_level achievement type';