-- Final elimination of ALL auto-achievement claiming
...
-- Step 2: Check ALL functions that might be related to achievements
SELECT 
  routine_name,
  routine_type
FROM information_schema.routines 
WHERE routine_schema = 'public' 
AND (routine_name LIKE '%achievement%' OR routine_name LIKE '%award%');

-- Step 3: Drop ALL possible triggers that might auto-award
...
-- Step 9: Check if there are any other functions that might insert into user_achievements
SELECT 
  routine_name,
  routine_type,
  routine_definition
...
-- Step 10: Check if there are any other functions that might call achievement functions
SELECT 
  routine_name,
  routine_type,
  routine_definition
FROM information_schema.routines 
WHERE routine_schema = 'public' 
AND routine_definition LIKE '%check_and_award_achievements%';

-- Step 11: Verify ALL triggers are disabled
SELECT 
  trigger_name,
  event_manipulation,
  action_statement,
...
-- Step 12: Verify ALL functions are disabled
SELECT 
  routine_name,
  routine_type
FROM information_schema.routines 
