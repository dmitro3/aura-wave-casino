-- Fix XP Calculation System with 3 Decimal Precision
-- $0.01 wager = 0.001 XP, $1.00 wager = 0.100 XP

-- Step 1: Update profiles table to support 3 decimal places for lifetime_xp
DO $$ 
BEGIN 
  IF EXISTS (
...
-- Step 2: Drop existing functions and recreate with correct formula
...
AS $function$
BEGIN
  -- XP = bet_amount / 10 (exactly 1/10 of wager)
  -- $0.01 bet = 0.001 XP
  -- $1.00 bet = 0.100 XP
...
-- Step 4: Create corrected add_xp_and_check_levelup function
...
-- Step 5: Create corrected trigger function
...
SELECT 
  ...
  public.calculate_xp_from_bet(0.01) as one_cent_xp,
  public.calculate_xp_from_bet(0.10) as ten_cents_xp,
  public.calculate_xp_from_bet(1.00) as one_dollar_xp,
  public.calculate_xp_from_bet(10.00) as ten_dollars_xp;

-- Expected results:
-- one_cent_xp = 0.001
-- ten_cents_xp = 0.010
-- one_dollar_xp = 0.100
-- ten_dollars_xp = 1.000