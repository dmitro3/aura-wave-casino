-- Complete the leveling system setup

-- Create the remaining tables for the enhanced leveling system
CREATE TABLE IF NOT EXISTS public.case_rewards (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  level_unlocked INTEGER NOT NULL,
  rarity TEXT NOT NULL CHECK (rarity IN ('common', 'rare', 'epic', 'legendary', 'pending')),
...
-- Insert border tier definitions (only if table is empty)
INSERT INTO public.border_tiers (tier, min_level, max_level, name, description, css_classes, animation_type) 
SELECT * FROM (VALUES
...
) AS v(tier, min_level, max_level, name, description, css_classes, animation_type)
WHERE NOT EXISTS (SELECT 1 FROM public.border_tiers);
