-- Fix Tower Game Permission Issues & Live Bet Feed Issues (CORRECTED VERSION)
...
-- Ensure realtime is enabled (only set replica identity, skip publication)
ALTER TABLE public.live_bet_feed REPLICA IDENTITY FULL;

-- Only add to publication if not already there (use DO block to handle error)
DO $$ 
BEGIN
