-- FINAL WORKING DATABASE RESTORE
-- This script restores all functions and tables needed by the roulette-engine
...
-- Create place_roulette_bet function (called by roulette-engine)
...
BEGIN
    ...
    -- Insert bet
    INSERT INTO public.roulette_bets (round_id, user_id, bet_color, bet_amount, potential_payout) 
    VALUES (p_round_id, p_user_id, p_bet_color, p_bet_amount, potential_payout)
...
-- Create complete_roulette_round function (called by roulette-engine)
...
BEGIN
    ...
    FOR bet_record IN SELECT * FROM public.roulette_bets WHERE round_id = p_round_id LOOP
        ...
        -- Update bet record
        UPDATE public.roulette_bets SET 
            is_winner = bet_record.is_winner,
            actual_payout = bet_record.actual_payout,
            profit = bet_record.profit
...
-- Create get_or_create_daily_seed function (called by roulette-engine)
...
BEGIN
    ...
    VALUES (p_date, server_seed, server_seed_hash, lotto, lotto_hash, false)
    RETURNING * INTO new_seed;
...
-- Create reveal_daily_seed function (called by roulette-engine)
...
BEGIN
    ...
    IF NOT seed_exists THEN
        INSERT INTO public.daily_seeds (date, server_seed, server_seed_hash, lotto, lotto_hash, is_revealed) 
        VALUES (today_date, encode(gen_random_bytes(32), 'hex'), encode(digest(encode(gen_random_bytes(32), 'hex'), 'sha256'), lpad(floor(random() * 10000000000)::text, 10, '0'), encode(digest(lpad(floor(random() * 10000000000)::text, 10, '0'), 'sha256'), false);
...
DO $$
BEGIN
    RAISE NOTICE 'Final working database restore finished successfully!';
