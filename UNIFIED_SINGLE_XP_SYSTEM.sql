-- UNIFIED SINGLE XP SYSTEM - One XP to Rule Them All
-- This creates a clean, unified XP system with one total XP value per user
-- Levels are calculated based on XP thresholds, no more confusing current/lifetime split
...
-- Insert level requirements (exponential growth)
INSERT INTO public.level_requirements (level, total_xp_required, xp_for_this_level) VALUES
(1, 0, 0),                    -- Level 1: 0 XP total
(2, 100, 100),                -- Level 2: 100 XP total (100 XP for level)
(3, 250, 150),                -- Level 3: 250 XP total (150 XP for level)
(4, 450, 200),                -- Level 4: 450 XP total (200 XP for level)
(5, 700, 250),                -- Level 5: 700 XP total (250 XP for level)
(6, 1000, 300),               -- Level 6: 1000 XP total (300 XP for level)
(7, 1350, 350),               -- Level 7: 1350 XP total (350 XP for level)
(8, 1750, 400),               -- Level 8: 1750 XP total (400 XP for level)
(9, 2200, 450),               -- Level 9: 2200 XP total (450 XP for level)
(10, 2700, 500),              -- Level 10: 2700 XP total (500 XP for level)
(11, 3250, 550),              -- Level 11: 3250 XP total (550 XP for level)
(12, 3850, 600),              -- Level 12: 3850 XP total (600 XP for level)
(13, 4500, 650),              -- Level 13: 4500 XP total (650 XP for level)
(14, 5200, 700),              -- Level 14: 5200 XP total (700 XP for level)
(15, 5950, 750),              -- Level 15: 5950 XP total (750 XP for level)
(16, 6750, 800),              -- Level 16: 6750 XP total (800 XP for level)
(17, 7600, 850),              -- Level 17: 7600 XP total (850 XP for level)
(18, 8500, 900),              -- Level 18: 8500 XP total (900 XP for level)
(19, 9450, 950),              -- Level 19: 9450 XP total (950 XP for level)
(20, 10450, 1000),            -- Level 20: 10450 XP total (1000 XP for level)
(21, 11500, 1050),            -- Level 21: 11500 XP total (1050 XP for level)
(22, 12600, 1100),            -- Level 22: 12600 XP total (1100 XP for level)
(23, 13750, 1150),            -- Level 23: 13750 XP total (1150 XP for level)
(24, 14950, 1200),            -- Level 24: 14950 XP total (1200 XP for level)
(25, 16200, 1250),            -- Level 25: 16200 XP total (1250 XP for level) + CASE REWARD
(26, 17500, 1300),            -- Level 26: 17500 XP total (1300 XP for level)
(27, 18850, 1350),            -- Level 27: 18850 XP total (1350 XP for level)
(28, 20250, 1400),            -- Level 28: 20250 XP total (1400 XP for level)
(29, 21700, 1450),            -- Level 29: 21700 XP total (1450 XP for level)
(30, 23200, 1500),            -- Level 30: 23200 XP total (1500 XP for level)
(35, 32700, 1750),            -- Level 35: 32700 XP total (1750 XP for level)
(40, 44200, 2000),            -- Level 40: 44200 XP total (2000 XP for level)
(45, 57700, 2250),            -- Level 45: 57700 XP total (2250 XP for level)
(50, 73200, 2500),            -- Level 50: 73200 XP total (2500 XP for level) + CASE REWARD
(60, 113200, 3000),           -- Level 60: 113200 XP total (3000 XP for level)
(70, 163200, 3500),           -- Level 70: 163200 XP total (3500 XP for level)
(80, 223200, 4000),           -- Level 80: 223200 XP total (4000 XP for level)
(90, 293200, 4500),           -- Level 90: 293200 XP total (4500 XP for level)
(100, 373200, 5000)           -- Level 100: 373200 XP total (5000 XP for level) + CASE REWARD
ON CONFLICT (level) DO UPDATE SET
  total_xp_required = EXCLUDED.total_xp_required,
  xp_for_this_level = EXCLUDED.xp_for_this_level;

-- Fill in missing levels with progressive requirements
WITH RECURSIVE level_generation AS (
  -- Start with existing levels
  SELECT level, total_xp_required, xp_for_this_level
  FROM public.level_requirements
  WHERE level <= 30
  
  UNION ALL
  
  -- Generate missing levels with progressive XP requirements
  SELECT 
    lg.level + 1,
    lg.total_xp_required + (lg.xp_for_this_level + 50), -- Increase XP requirement by 50 each level
    lg.xp_for_this_level + 50 -- Each level requires 50 more XP than the previous
  FROM level_generation lg
  WHERE lg.level < 100
)
INSERT INTO public.level_requirements (level, total_xp_required, xp_for_this_level)
SELECT level, total_xp_required, xp_for_this_level
FROM level_generation
WHERE level > 30 AND level <= 100
...
BEGIN
  ...
  SELECT lr.level, lr.total_xp_required, 
         COALESCE(next_lr.total_xp_required, lr.total_xp_required + 5000) -- Default next level requirement
  ...
  IF level_end_xp > level_start_xp THEN
    progress := (current_xp_in_level / (level_end_xp - level_start_xp)) * 100;
  ELSE
    progress := 100; -- Max level reached
  END IF;
  
  -- Ensure values are non-negative
  current_xp_in_level := GREATEST(current_xp_in_level, 0);
  xp_needed := GREATEST(xp_needed, 0);
  progress := GREATEST(LEAST(progress, 100), 0);
...
BEGIN
  ...
  new_total_xp := ROUND(COALESCE(old_total_xp, 0) + xp_amount, 3);
...
-- ==================================================
-- STEP 4: Update all existing user data to unified system
-- ==================================================

-- Update all users to have correct total_xp and calculated levels
UPDATE public.profiles p
SET 
  total_xp = GREATEST(COALESCE(p.lifetime_xp, 0), COALESCE(p.current_xp, 0), COALESCE(p.xp, 0)),
  current_level = calc.current_level,
  current_xp = calc.current_level_xp,
  xp_to_next_level = calc.xp_to_next_level,
  lifetime_xp = GREATEST(COALESCE(p.lifetime_xp, 0), COALESCE(p.current_xp, 0), COALESCE(p.xp, 0))
FROM public.calculate_level_from_total_xp(GREATEST(COALESCE(p.lifetime_xp, 0), COALESCE(p.current_xp, 0), COALESCE(p.xp, 0))) calc
...
FROM public.calculate_level_from_total_xp(150) -- Should be level 2
UNION ALL
SELECT 
...
FROM public.calculate_level_from_total_xp(1500) -- Should be level 7
UNION ALL
SELECT 
  'LEVEL CALCULATION TEST',
  total_xp,
  ...
  xp_to_next_level,
  ROUND(progress_percentage, 2)
FROM public.calculate_level_from_total_xp(10000); -- Should be level 19
